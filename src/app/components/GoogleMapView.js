...
import { getPedidos, updateEstadoContenedorDisponible, updateEstadoPedido, updateRemitoPedido } from '../firebase/firestore/firestore'
          ...
          <div className={`alert-box ${showAnimation ? 'animate' : ''}`}>
            <div className="px-8 py-6 bg-green-400 text-white flex justify-between rounded">
              <div className="flex items-center">
                ...
                <p>Bien! El pedido ha pasado a estado <span className='uppercase'>{proximoEstado}</span></p>
              </div>
              ...
              >
                <div className="relative overflow-hidden flex flex-col justify-between space-y-4 text-sm rounded-xl max-w-[23rem] p-4 bg-white text-slate-700 shadow-lg">
                  <div className="flex justify-between items-center">
                    <div className={`uppercase text-xs font-semibold ${selected.estado === "pendiente" ? 'text-yellow-500' : selected.estado === "entregado" ? 'text-green-400' : selected.estado === "retirar" ? 'text-orange-400' : selected.estado === "completado" ? 'text-blue-500' : ''}`}>
                      {selected.estado}
                    </div>
                    <div className="flex items-center">
               ...
               md:space-x-2 space-y-2 md:space-y-0 gap-2">
                    <button
                      onClick={() => {
                        selected.estado === "pendiente" ? handleOnClick(selected.contenedor, "entregado", true)
                          : selected.estado === "entregado" ? handleOnClick(selected.contenedor, "retirar", false)
                            : selected.estado === "retirar" ? handleOnClick(selected.contenedor, "completado", false)
                              : null
                      }}
                      className={`flex items-center justify-center px-4 py-2 text-sm text-white 
                      ${selected.estado === "pendiente" ? 'bg-green-500 hover:bg-green-600'
                          : selected.estado === "entregado" ? 'bg-orange-500 hover:bg-orange-600'
                            : selected.estado === "retirar" ? 'bg-blue-500 hover:bg-blue-600'
                              : null} rounded-lg transition duration-200`}
                    >
                      {selected.estado === "pendiente" ? "Entregado"
                        : selected.estado === "entregado" ? "Retirado"
                          : selected.estado === "retirar" ? "Completado"
                            : null}
                      <svg
